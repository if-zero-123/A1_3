...
// 配置参数（平衡低延迟 + 抗NPU噪声）
// ============================================================================
constexpr bool  kEnableFaceDetection = true;       // 脸部识别总开关：true开启，false关闭
...
constexpr int   kFaceInferIntervalStable = 6;       // 稳定后进一步降低刷新占用
...
constexpr int   kPoseInferIntervalFast = 3;               // 手势出现/切换阶段推理周期
constexpr int   kPoseInferIntervalStable = 5;             // 手势稳定后进一步降频
...
constexpr int   kPoseOsdDrawIntervalStable = 2;           // 手势稳定显示时隔帧重绘
...
struct EyeTrack {
    ...
    float MotionSpeed() const {
        return std::max(kcx.Speed(), kcy.Speed());
    }
};
...
bool ShouldRunEyeInference(int frame_id) {
    ...
    for (int i = 0; i < 2; ++i) {
        if (g_eye_tracks[i].IsReady()) {
            max_speed = std::max(max_speed, g_eye_tracks[i].MotionSpeed());
        }
    }
    if (max_speed >= kEyeTrackActiveSpeedPx) {
        return true;
    }
    return (frame_id % kEyeInferIntervalStable) == 0;
}

int GetPoseInferInterval(bool has_pose_overlay) {
    if (!has_pose_overlay || !g_pose_track.active) {
        return kPoseInferIntervalFast;
    }
    return g_pose_track.IsStable() ? kPoseInferIntervalStable : kPoseInferIntervalFast;
}

bool ShouldRunPoseInference(int frame_id, bool has_pose_overlay) {
    const int phase = frame_id % GetPoseInferInterval(has_pose_overlay);
    if (!has_pose_overlay || !g_pose_track.active) {
        return phase != 0;
    }
...
std::vector<std::array<float, 4>> GetPredictedEyeBoxes(int img_w, int img_h) {
    std::vector<std::array<float, 4>> stable;
    stable.reserve(2);
    for (int i = 0; i < 2; ++i) {
        if (!g_eye_tracks[i].IsReady()) {
...
bool ShouldRedraw(const std::vector<std::array<float, 4>>& boxes,
    ...
    const bool stable_pose_overlay =
        g_pose_track.active &&
        g_pose_track.IsStable() &&
        class_ids != nullptr &&
        !class_ids->empty();
    if (stable_pose_overlay &&
        (frame_id - g_last_draw_frame) < kPoseOsdDrawIntervalStable) {
        return false;
    }
...
void inference_thread_func(POSEDETGRAY* pose_detector,
    ...
    while (!stop_inference) {
        ...
        if (ShouldRunEyeInference(img_pair.frame_id)) {
            eye_detector->Predict(&img_pair.img1, eye_result, kEyeInferConfThreshold);
            stable_boxes = GetStableEyeBoxes(*eye_result,
                                             has_face_roi ? &last_face_roi : nullptr,
                                             y_plane,
                                             img_width,
                                             img_height);
        } else {
            stable_boxes = GetPredictedEyeBoxes(img_width, img_height);
        }
...
int main(int argc, char* argv[]) {
    ...
    printf("[INFO] Face detection: %s\n", kEnableFaceDetection ? "enabled" : "disabled");
    ...
    std::thread inference_thread(inference_thread_func,
                                 &pose_detector,
                                 &eye_detector,
                                 kEnableFaceDetection ? &face_detector : nullptr,
